...
public class EnableBarrierCommand implements SubCommand {

    private final TGSquidGame plugin;

    public EnableBarrierCommand(TGSquidGame plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (args.length < 1) {
            sender.sendMessage(ChatColor.RED + "Usage: /tgsg <arenaName> enablebarrier");
            return true;
        }
        ...
        arena.setBarrierEnabled(true);
        sender.sendMessage(ChatColor.GREEN + "Barriers enabled for arena '" + arenaName + "'");
