...
public class SetStart1Command implements SubCommand {

    private final TGSquidGame plugin;

    public SetStart1Command(TGSquidGame plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (args.length < 1) {
            player.sendMessage(ChatColor.RED + "Usage: /tgsg <arenaName> setstart1");
            return true;
        }
        ...
        player.sendMessage(ChatColor.YELLOW + "💾 Use /tgsg " + arenaName + " save to confirm changes");
