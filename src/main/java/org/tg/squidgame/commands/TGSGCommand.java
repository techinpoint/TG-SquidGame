    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 2) {
            ...
            if (plugin.getArenaManager().arenaExists(args[0])) {
                String arenaType = plugin.getArenaManager().getArena(args[0]).getType();
                
                // Common commands for all arena types
                completions.addAll(Arrays.asList("config", "edit", "save", "cancel", "lobby", 
                    "setlobby", "setspec", "setpos1", "setpos2", "join", "leave", "reload", "start", "stop"));
                
                // Type-specific commands
                if ("RedLightGreenLight".equalsIgnoreCase(arenaType)) {
                    completions.addAll(Arrays.asList("setstart1", "setstart2", "setwin1", "setwin2", "enablebarrier", "disablebarrier"));
                }
        ...
        if (args.length == 3 && "create".equals(args[0])) {
            completions.addAll(Arrays.asList("RedLightGreenLight", "Holi"));
