...
public class PlayerManager {
    ...
    private final Map<UUID, Collection<PotionEffect>> originalEffects;
    ...
    public PlayerManager(TGSquidGame plugin) {
        ...
        this.originalEffects = new HashMap<>();
    ...
    public void addPlayer(String arenaName, Player player) {
        ...
        // Store original state only once (on first join)
        if (!playerArenasMap.containsKey(uuid)) {
            ...
            originalEffects.put(uuid, new ArrayList<>(player.getActivePotionEffects()));
            originalFoodLevel.put(uuid, player.getFoodLevel());
            originalHealth.put(uuid, player.getHealth());
    ...
    private void restorePlayerState(Player player) {
        ...
        player.getActivePotionEffects().forEach(effect -> player.removePotionEffect(effect.getType()));
        Collection<PotionEffect> effects = originalEffects.remove(uuid);
        if (effects != null) {
            effects.forEach(player::addPotionEffect);
        }
        
        // Restore food level
        Integer foodLevel = originalFoodLevel.remove(uuid);
        if (foodLevel != null) {
            player.setFoodLevel(foodLevel);
        }
        
        // Restore health
        Double health = originalHealth.remove(uuid);
        if (health != null) {
