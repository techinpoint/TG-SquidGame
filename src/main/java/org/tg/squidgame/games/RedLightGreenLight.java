...
public class RedLightGreenLight {
    ...
    private int redLightGraceTicks = 0;
    private int countdownTimer;
    private boolean redLightWarning;
    private boolean waitingMessageShown;
    private static final int RED_LIGHT_WARNING_TICKS = 20;
    private static final int RED_LIGHT_GRACE_PERIOD_TICKS = 20;
    ...
    private void startGameLoop() {
        gameTask = new BukkitRunnable() {
            ...
            private int phaseLength = getRandomPhaseLength() * 20; // Convert seconds to ticks
            ...
            public void run() {
                ...
                // Show red light bossbar WARNING_TICKS (1 second) early during green light
                if (lightState == LightState.GREEN && phaseTicks >= phaseLength - RED_LIGHT_WARNING_TICKS && !redLightWarning) {
                ...
                if (phaseTicks >= phaseLength) {
                    toggleLight();
                    phaseTicks = 0;
                    phaseLength = getRandomPhaseLength() * 20;
                    redLightWarning = false;
                }

                // Handle red light checks with tick-based grace period
                if (lightState == LightState.RED) {
                    if (redLightGraceTicks > 0) {
                        redLightGraceTicks--;
                    } else {
                        checkPlayerMovement();
                    }
        ...
        }.runTaskTimer(plugin, 0L, 1L); // Run every tick for precise timing
    }

    private void toggleLight() {
        if (lightState == LightState.GREEN) {
            lightState = LightState.RED;
            redLightGraceTicks = RED_LIGHT_GRACE_PERIOD_TICKS; // Start 1-second grace period
            // Bossbar already set to red during warning; reinforce if needed
            bossBar.setTitle("🔴 RED LIGHT - STOP!");
            bossBar.setColor(BarColor.RED);
    ...
    private void checkPlayerMovement() {
        // Grace period handled in game loop; no need for time check here
        ...
        for (UUID uuid : players) {
            ...
            if (lastPos == null) {
                lastPos = player.getLocation().clone();
            ...
            public void run() {
                ...
                if (players.size() < arena.getMinPlayers()) {
                    if (!waitingMessageShown) {
                        broadcastMessage(ChatColor.YELLOW + "Waiting for players... Minimum " + arena.getMinPlayers() + " player(s) to start the game.");
    ...
    public void resetWaitingMessage() {
        waitingMessageShown = false;
    }
